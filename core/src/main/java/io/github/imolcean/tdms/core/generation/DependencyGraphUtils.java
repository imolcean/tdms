...
public class DependencyGraphUtils
{
    ...
    {
        File imgFile = new File(String.format("C:\\tdms\\graphs\\%s.png", name));
        while(!imgFile.createNewFile())
        {
            name = String.format("%s_copy", name);
            imgFile = new File(String.format("C:\\tdms\\graphs\\%s.png", name));
        }

        JGraphXAdapter<Table, DefaultEdge> adapter = new JGraphXAdapter<>(g);
